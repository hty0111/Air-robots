...
gain_pos = [0.5, 0.5, 1];
gain_vel = [1, 1, 1];
gain_angle = [1, 1, 1];     % phi theta yaw
gain_omiga = [1, 1, 1];
...
[phi, theta, yaw] = RotToRPY_ZXY(Rot);  % true angle

phi_des = 1 / g * (a_out(1)*sin(phi) - a_out(2)*cos(phi));
theta_des = 1 / g * (a_out(1)*cos(phi) + a_out(2)*sin(phi));
yaw_des = yaw;
wx_dot = gain_angle(1) * (phi_des - phi) + gain_omiga(1) * (0 - s(11));
wy_dot = gain_angle(2) * (theta_des - theta) + gain_omiga(2) * (0 - s(12));
wz_dot = gain_angle(3) * (yaw_des - yaw) + gain_omiga(3) * (0 - s(13));
